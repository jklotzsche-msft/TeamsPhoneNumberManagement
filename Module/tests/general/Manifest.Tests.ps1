Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\TeamsPhoneNumberManagement").Path
	$manifest = ((Get-Content "$moduleRoot\TeamsPhoneNumberManagement.psd1") -join "`n") | Invoke-Expression
